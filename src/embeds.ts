...
// Default embed
export const defaultEmbed = () => new Embed();
// .setColor();
// .setTimestamp();
...
export const helpEmbed = (cmd: Command) => {
    ...
    if (cmd.help)
        embed = embed.addField({ name: 'Syntax', value: `\`${config.prefix}${cmd.names[0]} ${cmd.help.getSyntax()}\``, inline: true });
    if (Array.isArray(cmd.names))
        embed = embed.addField({ name: 'Aliases', value: cmd.names.join(', '), inline: true });
